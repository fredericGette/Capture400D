 ...
 *
 * @version $Id: Container.java,v 1.7 2000/10/06 00:59:13 dbrownell Exp $
...
abstract public class Container extends Buffer
{
    ...
    //	@ 8, u32 xid
    // TOTAL:  12 bytes
    public static final int	HDR_LEN = 12;

    protected Container (byte buf [])
	{ super (buf, buf.length); }

    Container (byte buf [], int len)
	{ super (buf, len); }
	...
	switch (code >> 12) {
	    ...
	    default:	return Integer.toHexString (code).intern ();
	}
    }
     ...
     * permit type-specific and vendor-specific subclasses to name their
